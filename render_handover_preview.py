...
from build_handover import DEMO, COND_FILL, HOUSES_DEMO

NAVY = "#1F4E78"
HOUSE_BAT = dict(HOUSES_DEMO)
...
cols = [("מס׳", 0.5), ("בית / מוצב", 1.1), ("גדוד מוסר", 1.1), ("קטגוריה", 1.6),
        ("שם הפריט", 2.2), ("פריט צ׳?", 0.9), ("מספר צ׳", 1.0), ("מקור", 1.0),
        ("כמות נמסרת", 1.0), ("מצב", 1.1), ("נבדק (קולט)", 1.1), ("פער", 0.6),
        ("הערות", 2.0)]
...
for i, (house, cat, item, tz, tznum, src, qty, cond) in enumerate(DEMO):
    ...
    vals = [str(i + 1), house, HOUSE_BAT.get(house, ""), cat, item, tz, tznum,
            src, str(qty), cond, str(checked), str(gap), ""]
    bgs = ["#FFFFFF", "#FFFFFF", "#EAF1FA", "#FFFFFF", "#F4F7FC", tz_bg,
           "#FFFFFF", "#FFFFFF", "#FFFFFF", cond_bg, "#FFFFFF", gap_bg, "#FFFFFF"]
    for (lab, w), xx, v, bg in zip(cols, xs, vals, bgs):
        cell(xx, y, w, v, bg, bold=(lab in ("פער", "מצב")))

kpi = "הגדודים המוסרים ממלאים בית→מצב  |  'גדוד מוסר' נשלף אוטומטית לפי הבית  |  הקולט ממלא 'נבדק' והפער מחושב לבד"
